...
import type { ImmobilierContent, ImmobilierTestimonial } from "@/data/immobilier";
import { SectionTitle } from "@/components/ui/SectionTitle";
import { Reveal } from "@/components/showroom/Reveal";

interface TestimonialsSectionProps {
  content: ImmobilierContent["testimonials"];
}

export function TestimonialsSection({ content }: TestimonialsSectionProps) {
  return (
    <section id="avis" className="scroll-mt-20 border-b border-brand-line bg-brand-charcoal py-16 sm:py-20">
...
function TestimonialCard({ testimonial }: { testimonial: ImmobilierTestimonial }) {
  return (
    <figure className="flex h-full flex-col rounded-xl border border-brand-line bg-brand-black p-6 shadow-sm">
      ...
      <figcaption className="mt-5 border-t border-brand-line pt-4 text-sm">
        ...
        {testimonial.context ? <span className="block text-brand-silver">{testimonial.context}</span> : null}
      </figcaption>
    </figure>
  );
}
