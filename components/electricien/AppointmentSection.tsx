...
import type { ElectricienContent, ElectricienService } from "@/data/electricien";
import { SectionTitle } from "@/components/ui/SectionTitle";
import { ContactValue } from "@/components/showroom/ContactValue";
import { ElectricienQuoteForm } from "@/components/forms/ElectricienQuoteForm";
...
interface AppointmentSectionProps {
  content: ElectricienContent["appointment"];
  services: ElectricienService[];
  defaultInterventionSlug?: string;
}

export function AppointmentSection({ content, services, defaultInterventionSlug }: AppointmentSectionProps) {
  const mapsUrl = externalHref(siteConfig.address.mapsUrl);

  return (
    <section id="devis" className="scroll-mt-20 bg-brand-charcoal py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-14">
          ...
          <ElectricienQuoteForm services={services} defaultInterventionSlug={defaultInterventionSlug} />
        </div>
      </div>
    </section>
  );
