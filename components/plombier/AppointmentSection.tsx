...
import type { PlombierContent, PlombierService } from "@/data/plombier";
import { SectionTitle } from "@/components/ui/SectionTitle";
import { ContactValue } from "@/components/showroom/ContactValue";
import { PlombierQuoteForm } from "@/components/forms/PlombierQuoteForm";
...
interface AppointmentSectionProps {
  content: PlombierContent["appointment"];
  services: PlombierService[];
  defaultProblemSlug?: string;
}

export function AppointmentSection({ content, services, defaultProblemSlug }: AppointmentSectionProps) {
    ...
    <section id="devis" className="scroll-mt-20 bg-brand-charcoal py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-14">
          ...
          <PlombierQuoteForm services={services} defaultProblemSlug={defaultProblemSlug} />
        </div>
      </div>
    </section>
  );
