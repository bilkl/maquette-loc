...
import { ActivityLevelInput, BilanFormErrors, BilanFormValues } from "@/types/booking";
import { validateBilanForm } from "@/lib/validation";
import { submitBilanRequest } from "@/lib/booking-actions";
...
const objectiveOptions = [
  ...
  { value: "gestion-fatigue", label: "Gestion de la fatigue" },
  { value: "autre", label: "Autre objectif" },
];

const activityLevelOptions: { value: ActivityLevelInput; label: string }[] = [
  { value: "sedentaire", label: "Sédentaire — peu ou pas d'activité" },
  { value: "peu-actif", label: "Peu actif — activité occasionnelle" },
  { value: "actif", label: "Actif — activité régulière" },
];

const initialValues: BilanFormValues = {
  firstName: "",
  lastName: "",
  email: "",
  phone: "",
  objective: "",
  activityLevel: "peu-actif",
  preferredDate: "",
...
/**
 * Formulaire "Réserver mon bilan gratuit" : l'objectif principal et le
 * niveau d'activité actuel suffisent à préparer le premier échange — pas de
 * questionnaire médical détaillé sur ce formulaire, volontairement gardé
 * simple pour ne pas décourager une prise de contact.
 */
export function BilanForm() {
  const [values, setValues] = useState<BilanFormValues>(initialValues);
  const [errors, setErrors] = useState<BilanFormErrors>({});
  ...
  function updateField<K extends keyof BilanFormValues>(field: K, value: BilanFormValues[K]) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const validationErrors = validateBilanForm(values);
    ...
    try {
      const result = await submitBilanRequest(values);
      if (result.success) {
  ...
  const objectiveLabel = objectiveOptions.find((option) => option.value === values.objective)?.label ?? "un objectif";
  const recapMessage = `Bonjour ${siteConfig.name}, je souhaite réserver mon bilan gratuit. Mon objectif principal : ${objectiveLabel}.`;
  ...
  if (successMessage) {
    return (
      <div role="status" className="rounded-xl border border-brand-accent/40 bg-brand-accent/5 p-8">
        ...
        <p className="mt-5 text-xl font-black text-brand-ivory">Demande envoyée.</p>
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <Field id={`${formId}-objective`} label="Objectif principal" error={errors.objective} required>
          <select
            id={`${formId}-objective`}
        ...
        <Field id={`${formId}-activityLevel`} label="Niveau d'activité actuel" required>
          <select
            id={`${formId}-activityLevel`}
            value={values.activityLevel}
            onChange={(event) => updateField("activityLevel", event.target.value as ActivityLevelInput)}
        ...
        <Field id={`${formId}-date`} label="Disponibilité souhaitée (facultatif)" error={errors.preferredDate}>
          <input
            id={`${formId}-date`}
            type="date"
            min={minDate}
        ...
        <Field id={`${formId}-time`} label="Créneau préféré">
          <select
            ...
            onChange={(event) => updateField("preferredTime", event.target.value as BilanFormValues["preferredTime"])}
            className={inputClass(false)}
          >
      ...
      <div className="mt-5">
        <Field id={`${formId}-message`} label="Précisions (facultatif)">
          <textarea
            id={`${formId}-message`}
            rows={3}
            placeholder="Contraintes physiques, blessures, contexte particulier…"
      ...
      <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <CoachButton type="submit" disabled={isSubmitting} className="w-full sm:w-auto">
          {isSubmitting ? (
            <>
