...
import type { ElectricienService } from "@/data/electricien";
import { ElectricienQuoteFormErrors, ElectricienQuoteFormValues } from "@/types/booking";
import { validateElectricienQuoteForm } from "@/lib/validation";
import { submitElectricienQuoteRequest } from "@/lib/booking-actions";
import { todayIso } from "@/lib/utils";
import { getWhatsAppUrl, isWhatsAppEnabled } from "@/lib/whatsapp";
import { WhatsAppIcon } from "@/components/ui/icons";
import { ElectricienButton } from "@/components/electricien/ElectricienButton";
import { cn } from "@/lib/utils";

const OTHER_INTERVENTION = "autre";

const initialValues: ElectricienQuoteFormValues = {
  firstName: "",
  lastName: "",
  email: "",
  phone: "",
...
interface ElectricienQuoteFormProps {
  services: ElectricienService[];
  /** Pré-sélectionne un type d'intervention (ex. depuis /prestations#<slug>) */
  defaultInterventionSlug?: string;
}

/**
 * Formulaire de devis "intelligent" : le type d'intervention vient directement
 * du catalogue de prestations (pas de texte libre à deviner), une case
 * "urgence" dispense de choisir une date/un créneau — une urgence n'attend
 * pas un planning — et un récapitulatif WhatsApp pré-rempli est proposé pour
 * ceux qui préfèrent finaliser par message plutôt que par formulaire.
 */
export function ElectricienQuoteForm({ services, defaultInterventionSlug }: ElectricienQuoteFormProps) {
  const [values, setValues] = useState<ElectricienQuoteFormValues>({
    ...initialValues,
    interventionType: defaultInterventionSlug ?? "",
  });
  const [errors, setErrors] = useState<ElectricienQuoteFormErrors>({});
  ...
  function updateField<K extends keyof ElectricienQuoteFormValues>(
    field: K,
    value: ElectricienQuoteFormValues[K],
  ) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const validationErrors = validateElectricienQuoteForm(values);
    ...
    try {
      const result = await submitElectricienQuoteRequest(values);
      if (result.success) {
  ...
  const selectedIntervention = services.find((service) => service.slug === values.interventionType);
  const interventionLabel = selectedIntervention?.name ?? "une intervention";
  const timeLabel =
  ...
  const recapMessage = values.isUrgent
    ? `Bonjour ${siteConfig.name}, j'ai une urgence : ${interventionLabel}. Merci de me rappeler rapidement.`
    : `Bonjour ${siteConfig.name}, je souhaite un devis pour ${interventionLabel}${
    ...
    >
      <label
        className={cn(
          ...
          values.isUrgent
            ? "border-amber-500 bg-amber-500/10 text-brand-ivory"
            : "border-brand-line text-brand-silver hover:border-amber-500/60",
        )}
      >
        <input
          type="checkbox"
          ...
          className="mt-1 h-4 w-4 shrink-0 rounded border-brand-line accent-amber-500"
        />
        <span className="flex items-start gap-2">
          <Siren className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" aria-hidden="true" />
          C&apos;est une urgence — pas besoin de proposer une date, contactez-moi dès que possible.
        </span>
      </label>

      <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2">
        <Field
          id={`${formId}-interventionType`}
        ...
        >
          <select
            ...
            onChange={(event) => updateField("interventionType", event.target.value)}
            aria-invalid={!!errors.interventionType}
            className={inputClass(!!errors.interventionType)}
          >
            ...
            <option value={OTHER_INTERVENTION}>Autre / je ne sais pas encore</option>
          </select>
        </Field>
            ...
            <Field id={`${formId}-time`} label="Créneau préféré">
              <select
                ...
                onChange={(event) =>
                  updateField(
                    "preferredTime",
                    event.target.value as ElectricienQuoteFormValues["preferredTime"],
                  )
                }
      ...
      <div className="mt-5">
        <Field id={`${formId}-message`} label="Précisions (facultatif)">
          <textarea
            id={`${formId}-message`}
            rows={3}
            placeholder="Symptôme, type de bâtiment, contexte du projet…"
      ...
      <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <ElectricienButton type="submit" disabled={isSubmitting} className="w-full sm:w-auto">
          {isSubmitting ? (
            <>
