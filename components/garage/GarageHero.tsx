...
import { Check, Phone, Wrench, PaintBucket } from "lucide-react";
...
/** Icônes décoratives disponibles en filigrane (voir `watermarkIcon`). */
...
interface GarageHeroProps {
  ...
  /**
   * Icône décorative en filigrane. Un identifiant plutôt qu'un composant :
   * `GarageHero` est un composant client, un composant Lucide passé en prop
   * depuis un composant serveur casserait la sérialisation RSC.
   */
  watermarkIcon?: keyof typeof watermarkIcons;
}
 ...
 */
export function GarageHero({ content, watermarkIcon = "wrench" }: GarageHeroProps) {
  const WatermarkIcon = watermarkIcons[watermarkIcon];
